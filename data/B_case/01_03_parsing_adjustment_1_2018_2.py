"""
01_03_parsing_adjustment_1_2018_2.py
---------------------------------
KCA 서비스·집단 분쟁조정 사례집(2018) PDF를 구조 파싱해서 JSON으로 저장합니다.

✅ 목표
1) 텍스트 PDF: pdfplumber로 텍스트 추출 → 사건번호 기준 케이스 묶기 → 섹션 분리
2) 스캔/이미지 PDF: (페이지 렌더링 → OpenAI Vision 페이지별 텍스트 추출) → 동일 파서 재사용
3) 페이지번호: 문서 하단의 페이지 번호 추출 (예: "제3장 ... 17", "18 2018 서비스")
4) OCR 캐시: PDF 1개당 캐시 파일 1개로 저장 (파일 폭증 방지)

✅ 섹션 구조 (KCA 2018 서비스·집단)
- "주 문" → order 필드
- "이 유" > "1. 기초 사실" 또는 "1. 사건 개요" → overview 필드
- "이 유" > "2. 판 단" 또는 "3. 판 단" → reason 필드
...
# 페이지 번호 감지: "18 2018 서비스" 또는 "제3장 ... 17" 형태
RE_PRINTED_PAGE = re.compile(
    r"(?m)^.*?(\d{2,3})\s*(?:2018|2017|물품|서비스|집단|의료|분쟁|조정).*?$|^(\d{2,3})\s*\|\s*2018"
)
...
def main():
    """메인 실행"""
    # 특정 파일만 처리
    target_name = "2018년 서비스·집단 분쟁조정 사례집.pdf"
