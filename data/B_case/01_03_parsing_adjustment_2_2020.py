"""
2020년 전자거래 조정사례 Gemini API 파싱 (2019년 사례 포함)

주요 기능:
- Gemini API를 사용한 PDF 파싱
- 자동 케이스 경계 탐색 ("사례1", "사례2" 형식)
- 두 가지 케이스 타입 처리: 조정 사례, 사무국 합의권고 사례
- number 필드: 숫자(정수)로 저장 (예: 1, 2, 3...)
- judgement 처리 규칙:
  * 조정 사례: dict 형태 {reason, order}
  * 합의권고 사례: 문자열 형태
...
2. PDF 경로 확인
3. python 01_03_parsing_adjustment_2_2020.py 실행
"""
import fitz
import json
import os
...
def find_case_boundaries(pdf_path):
    """케이스 경계 자동 탐색 - 2020년 패턴: "사례1", "사례2" 형식 (공백 없음) + 섹션 구분"""
    doc = fitz.open(pdf_path)
    boundaries = []
    section_markers = {}  # page_num -> section_type (조정 or 합의권고)

    # 2020년 패턴: "사례1", "사례2" 형식 (공백 없음)
    case_pattern = re.compile(r'사례\s*\d+')
    section1_pattern = re.compile(r'^I\s*$')  # "I" 단독
    section2_pattern = re.compile(r'^II\s*$')  # "II" 단독
    ...
    for page_num in range(1, total_pages):
        ...
        for line in lines[:20]:
            ...
            if section1_pattern.match(line_stripped):
                section1_start = page_num
                print(f"  조정 사례 섹션 시작: Page {page_num}")
            if section2_pattern.match(line_stripped):
    ...
    for page_num in range(1, total_pages):
        ...
        if section2_start and page_num >= section2_start:
            ...
            current_section = "조정"  # 기본값

        for line in lines[:20]:
            line_stripped = line.strip()
            # "사례1", "사례2" 패턴 매칭
            if case_pattern.search(line_stripped):
                # 이미 추가된 페이지는 스킵
                if page_num not in boundaries:
                    ...
                    try:
                        ...
                        print(f"  Page {page_num} [{current_section}]: [한글 출력 제한]")
                    break
                break
...
def parse_case_with_gemini(images, case_type="조정"):
    ...
    if case_type == "합의권고":
        prompt = """이 이미지들은 2020년 전자거래 분쟁조정 사례집의 사무국 합의권고 사례입니다.
...

{
  ...
  "overview": "1) 사건 개요 섹션의 원문 전체 (섹션 헤더 '1) 사건 개요' 포함)",
  "claims": {
    "applicant": ["2) 양 당사자 주장 섹션의 가. 신청인 주장 원문 전체"],
    "respondent": ["2) 양 당사자 주장 섹션의 나. 피신청인 주장 원문 전체"]
  },
  "judgement": "3) 합의권고 판단 섹션 전체 원문 (섹션 헤더 '3) 합의권고 판단' 포함, 모든 내용 포함)",
  "result": "4) 처리결과 섹션 원문 (섹션 헤더 '4) 처리결과' 포함, 예: '합의종결', '조정이첩' 등)"
}

**중요 지침**:
1. 모든 내용을 원문 그대로 추출 (요약, 의역, 표준화 절대 금지)
2. 줄바꿈과 문단 구조 유지
3. 섹션 헤더(1), 2), 3), 4), 가., 나. 등)를 반드시 포함
4. 빠진 내용 없이 완전하게 추출
5. case_number가 명시되지 않으면 빈 문자열로
6. method가 없으면 null로 설정
7. judgement는 "3) 합의권고 판단" 섹션 전체를 헤더부터 포함
8. result는 "4) 처리결과" 섹션을 헤더부터 포함하여 원문 그대로 추출
9. JSON 형식을 정확히 지켜주세요
10. 특수문자나 따옴표는 적절히 이스케이프 처리"""
    else:  # 조정
        prompt = """이 이미지들은 2020년 전자거래 분쟁조정 사례집의 조정 사례입니다.
다음 JSON 형식으로 **원문 그대로** 추출하세요:

{
  ...
  "judgement": {
    "reason": "3) 조정부 판단 섹션 원문 전체 (섹션 헤더 '3) 조정부 판단' 포함, 모든 하위 내용 포함)",
    "order": "4) 조정 결과 섹션 원문 전체 (섹션 헤더 '4) 조정 결과' 또는 '4) 조정결정' 포함)"
  },
  "result": null
}
...
5. case_number가 명시되지 않으면 빈 문자열로
6. method가 없으면 null로 설정
7. judgement는 dict 형태로 reason("3) 조정부 판단" 섹션 전체)과 order("4) 조정 결과" 섹션 전체)를 분리
8. result는 2020년 조정 사례에는 없으므로 반드시 null로 설정
9. JSON 형식을 정확히 지켜주세요
10. 특수문자나 따옴표는 적절히 이스케이프 처리"""
...
def process_judgement_field(case_data):
    ...
    # 조정 사례: judgement를 dict 형태로 유지, result는 null
    if case_type == '조정':
        ...
        # 2020년 조정 사례는 result가 항상 null
        case_data['result'] = None
...
def main():
    pdf_path = Path("01_B_parsed/02_02_AdjustmentCase/02_kisa/2020년 전자거래 분쟁조정 사례집.pdf")
    output_path = Path("01_B_parsed/02_02_AdjustmentCase/02_kisa/2020년 전자거래 조정사례.json")
    ...
    print("="*80)
    print("2020년 전자거래 조정사례 Gemini API 파싱")
    print("="*80)
