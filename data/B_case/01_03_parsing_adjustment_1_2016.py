"""
01_03_parsing_adjustment_1_2016.py
---------------------------------
KCA 일반·집단 분쟁조정 사례집(2016) PDF를 구조 파싱해서 JSON으로 저장합니다.

✅ 목표
1) 텍스트 PDF: pdfplumber로 텍스트 추출 → 사건번호 기준 케이스 묶기 → 섹션 분리
2) 스캔/이미지 PDF: (페이지 렌더링 → OpenAI Vision 페이지별 텍스트 추출) → 동일 파서 재사용
3) 페이지번호: 문서 하단의 페이지 번호 추출 (예: "제3장 ... 17", "18 2016 일반")
...
# 페이지 번호 감지: "18 2016 일반" 또는 "제3장 ... 17" 형태
RE_PRINTED_PAGE = re.compile(
    r"(?m)^.*?(\d{2,3})\s*(?:2016|2015|일반|집단|분쟁|조정).*?$|^(\d{2,3})\s*\|\s*2016"
)
...
def detect_printed_page(text: str) -> Optional[int]:
    ...
    # 패턴 2: "18 2016 일반"
    m2 = re.search(r"^(\d{2,3})\s+2016", tail.split('\n')[-1] if '\n' in tail else tail)
    if m2:
        try:
...
def parse_pdf(pdf_path: Path) -> Dict:
    ...
    for i, g in enumerate(groups):
        ...
        if parsed:
            cases.append(parsed)
            print(f"  [OK] Case {i+1}: {parsed.get('title', 'N/A')[:50]}")
...
def main():
    """메인 실행"""
    # 특정 파일만 처리
    target_name = "2016년 일반·집단 분쟁조정 사례집.pdf"
