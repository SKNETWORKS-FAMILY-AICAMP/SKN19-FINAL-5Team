...
# 설정
BASE_URL = "https://www.consumer.go.kr/user/ftc/consumer/dmgerlifcase/115"
LIST_URL = f"{BASE_URL}/selectDmgeRlifCaseList.do"
DETAIL_URL = f"{BASE_URL}/selectDmgeRlifCaseView.do"
ITEMS_PER_PAGE = 25
TOTAL_ITEMS = 1497  # 총 게시글 수
TOTAL_PAGES = (TOTAL_ITEMS + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE  # 60 페이지
MAX_RETRIES = 3
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "solution_case_2.json")
        ...
        try:
            ...
            for row in rows:
                ...
                if len(cols) >= 4:
                    ...
                    if link and 'dmgeRlifCaseSn=' in link.get('href', ''):
                        case_id = link.get('href').split('dmgeRlifCaseSn=')[1].split('&')[0]
...
def crawl_all():
    ...
    print(f"✓ 정렬 완료 (number: {all_data_sorted[0]['number']} → {all_data_sorted[-1]['number']})")
