...
2. PDF 경로 확인
3. python 01_03_parsing_adjustment_2_2023.py 실행
"""
import fitz
import json
import os
...
def find_case_boundaries(pdf_path):
    """케이스 경계 자동 탐색 - 2023년 패턴: "사례 1", "사례 2" 형식"""
    doc = fitz.open(pdf_path)
    boundaries = []

    # 2023년 패턴: "사례 1", "사례 2" 형식
...
- JSON 형식을 정확히 지켜주세요"""
        ...
        prompt = """이 이미지들은 2023년 전자거래 분쟁조정 사례집의 조정 사례입니다.
다음 JSON 형식으로 **원문 그대로** 추출하세요:

{
  ...
  "result": "4. 처리결과 섹션 전체 원문 (섹션 제목 '4. 처리결과' 포함, 한 글자도 바꾸지 말고 정확히 그대로)"
}
...
8. result는 "4. 처리결과" 섹션을 한 글자도 바꾸지 말고 그대로 추출 (예: "조정안 수락", "조정안 불수락" 등 원문 그대로)
...
def main():
    pdf_path = Path("01_B_parsed/02_02_AdjustmentCase/02_kisa/2023년 전자거래 분쟁조정 사례집.pdf")
    output_path = Path("01_B_parsed/02_02_AdjustmentCase/02_kisa/2023년 전자거래 조정사례.json")
    ...
    print("="*80)
    print("2023년 전자거래 조정사례 Gemini API 파싱")
    print("="*80)
    ...
    result = {
        ...
        "parsing_statistics": {
            ...
            "recommendation_cases": 0,  # 2023년은 권고 없음
