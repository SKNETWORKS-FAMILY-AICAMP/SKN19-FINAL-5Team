"""
[별표 2] 품목별 해결기준 2 PDF 파싱 스크립트 (Google Gemini Vision)
...
# 설정
PDF_PATH = r"C:\Users\Playdata\Desktop\project\05_5th_project\data_n_db\raw\02_Guide\소비자분쟁해결기준_별표\[별표 2] 품목별 해결기준 2.pdf"
# 출력 디렉토리: PDF와 같은 폴더
...
# Page offset (원본 PDF가 1개 파일이었으나 2개로 분할됨. 첫 번째 파일이 37페이지)
PAGE_OFFSET = 37
...
2. **대분류/중분류 처리 - 매우 중요!**:
   - **대분류**: "Ⅰ. 상품(재화)", "Ⅱ. 서비스업" 등이 로마숫자로 명시된 경우에만 입력
   - **중분류**: 페이지 상단이나 표 위쪽에 "1. xxx", "2. xxx", "3. xxx" 형태로 명시된 경우에만 입력
     * 중분류는 반드시 아라비아 숫자 + 점(.) + 공백으로 시작합니다 (예: "1. 전기.가스.방송.통신서비스 관련")
     * 중분류는 연속적입니다: 1 → 2 → 3 → 4 → 5...
     * 소분류(표 제목)와 혼동하지 마세요. 소분류는 보통 ①②③ 원문자로 시작하거나 다른 형태입니다
   - **명시되지 않으면**: 빈 문자열로 설정 (이전 페이지 값을 계승한다는 의미)
   - **주의**: 중분류가 "1. → 2. → 8."처럼 건너뛰면 안 됩니다. 반드시 연속적이어야 합니다!
...
7. **비고 키워드 매핑 및 참조 처리 - 매우 중요!**:
   ...
   - **중요: 분쟁유형 참조 처리**
     * 비고에 "(피해유형1)", "(피해유형2)", "(피해유형3)" 같은 참조가 있으면, 해당 번호의 분쟁유형 id의 notes에 매핑하세요
     * 예: "* (피해유형3)의 경우 ..."라는 비고는 → id가 "3"인 분쟁유형의 notes 배열에 추가
     * 절대 다른 분쟁유형의 conditions 안에 넣지 마세요!
...
**중분류/소분류 구분 예시**:
- 페이지에 "1. 전기.가스.방송.통신서비스 관련" 텍스트가 있고 그 아래 "①전기서비스" 표가 있다면:
  → 중분류 = "1. 전기.가스.방송.통신서비스 관련", 소분류 = "①전기서비스"
- 페이지에 "3. 공연․콘텐츠 관련" 텍스트가 있고 그 아래 "①공연업 (영화 및 비디오물 상영업 제외)" 표가 있다면:
  → 중분류 = "3. 공연․콘텐츠 관련", 소분류 = "①공연업 (영화 및 비디오물 상영업 제외)"
- 페이지에 중분류 표시 없이 바로 "②가스서비스" 표만 있다면:
  → 중분류 = "" (빈 문자열, 이전 페이지 값 계승), 소분류 = "②가스서비스"
"""
...
def parse_page_with_gemini(image_data: str, page_num: int, client) -> dict:
    ...
    try:
        ...
        # 각 테이블에 source_page 추가 (PAGE_OFFSET 적용)
        for table in tables:
            table["source_page"] = page_num + PAGE_OFFSET
        ...
        return [{
            "source_page": page_num + PAGE_OFFSET,
...
def parse_pdf(pdf_path: str, output_dir: str, start_page: int = 1, end_page: int = None) -> dict:
    ...
    results = {
        ...
        "parsing_metadata": {
            ...
            "total_pages": len(images) + PAGE_OFFSET,  # 전체 PDF 기준
            "parsed_pages": f"{start_page + PAGE_OFFSET}-{end_page + PAGE_OFFSET}",  # 실제 페이지 번호
    ...
    # 중분류 수정: 잘못된 중분류 제거 및 정리
    print("\n중분류 수정 시작...")
    last_중분류 = ""
    for idx, table in enumerate(results["data"]):
        if "error" in table:
            continue

        중분류 = table.get("중분류", "")

        # "비고란", "페이지" 같은 잘못된 중분류 제거
        if "비고란" in 중분류 or "페이지" in 중분류:
            print(f"  [FIX] Table {idx} (Page {table['source_page']}): 잘못된 중분류 제거: \"{중분류}\"")
            table["중분류"] = ""
            중분류 = ""
        ...
        if 중분류:
            last_중분류 = 중분류
            print(f"  [INFO] Page {table['source_page']}: 중분류 변경 = \"{중분류}\"")
        else:
            # 중분류가 없으면 이전 값 계승
            table["중분류"] = last_중분류
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("소비자분쟁해결기준 PDF 파싱 (Google Gemini 2.5 Flash Vision)")
    print("=" * 80)
    print()
