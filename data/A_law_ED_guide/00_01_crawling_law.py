"""
법령 크롤링 스크립트
법제처 국가법령정보센터에서 법령 조문을 크롤링하여 JSON으로 저장
"""
...
def load_law_urls(file_path):
    """법률_URL.txt 파일에서 법령명과 URL 목록 로드"""
    laws = []
...
def extract_law_info(page):
    ...
    try:
        ...
        if info_elem:
            ...
            # 법령번호 추출: [법률 제XXXXX호, ...]
            law_num_match = re.search(r'\[법률\s*제(\d+)호', info_text)
            if law_num_match:
                info["법령번호"] = f"법률 제{law_num_match.group(1)}호"
    except:
        pass
...
def crawl_law(law_name, url, output_dir="raw/01_law_ED"):
    ...
    with sync_playwright() as p:
        ...
        # 구조가 있는 법령 처리
        if has_structure:
            current_structure_path = []  # 현재 구조 경로 추적
...
def main():
    """메인 함수: 모든 법령 크롤링"""
    # URL 목록 파일 경로
    url_file = "raw/01_law_ED/01_법률_URL.txt"
    output_dir = "raw/01_law_ED"

    # 법령 목록 로드
    laws = load_law_urls(url_file)
    print(f"총 {len(laws)}개 법령 크롤링 예정")
