...
5. [방법 C: 스냅샷에서 복원 (데이터 복구)](#방법-c-스냅샷에서-복원-데이터-복구)
6. [pgvector 확장 설치](#pgvector-확장-설치)
7. [스키마 및 데이터 삽입](#스키마-및-데이터-삽입)
8. [팀원과 공유](#팀원과-공유)
9. [비용 관리](#비용-관리)
...
**⚠️ 중요: pgvector + 대용량 데이터(52,000+ 건) 사용 시 프리티어는 부족합니다!**
...
**프로덕션 권장 (ddoksori 프로젝트용)**:
- ⭐ **db.r7g.xlarge** (4 vCPU, 32GB RAM)
- pgvector + 대량 데이터에 최적
- 비용: ~$280/월
- Graviton 프로세서 (Intel r7i보다 20% 저렴)

**중간 옵션**:
- **db.r6g.large** (2 vCPU, 16GB RAM, ~$140/월)
- **db.t3.medium** (2 vCPU, 4GB RAM, ~$60/월)
...
#### 스토리지 유형
- ⭐ **범용 SSD (gp3)** 선택 (권장)
- gp3는 gp2보다 20% 저렴하고 성능 30배 빠름

**gp3 vs gp2 비교**:
| 항목 | gp3 (권장) | gp2 |
|------|-----------|-----|
| 기본 IOPS | 3,000 | 용량 비례 (100GB=300) |
| 가격 (100GB) | $11.5/월 | $13.8/월 |
| pgvector 적합성 | ✅ 매우 적합 | △ 느림 |

#### 할당된 스토리지
- ⭐ **100 GiB** (ddoksori 프로젝트용)
- pgvector 인덱스 + 52,000건 데이터에 필요
- 20GB는 부족 (데이터 + 인덱스 + 로그 = 약 30GB+)

**소규모 테스트용**:
- 20 GiB (프리티어 범위)

#### 프로비저닝된 IOPS (gp3 선택 시)
- 기본값 **3000** 사용
- 추가 비용 없음
...
# --db-instance-class: db.r7g.xlarge (4 vCPU, 32GB RAM, pgvector용)
# --allocated-storage: 100GB (pgvector 인덱스 + 데이터용)
# --storage-type: gp3 (3000 IOPS, gp2보다 빠르고 저렴)
...
## 방법 C: 스냅샷에서 복원 (데이터 복구)

**데이터가 날아갔지만 스냅샷이 있는 경우 사용합니다!**

스냅샷에서 복원하면 방법 A/B의 1-11단계를 건너뛰고 **기존 데이터와 설정을 그대로 복원**할 수 있습니다.

**✅ 사용 가능한 스냅샷** (2026-01-28 기준):
- **스냅샷 이름**: `ddoksori-final-2026-01-28-ver2`
- **데이터 버전**: v3.1.0
- **총 데이터**: 38,680건 (law_guide 6,077 + case 32,603)
- **스키마 버전**: PostgreSQL 17.2 + pgvector 0.8.1
- **기능**: 제목 가중치 시스템, 조문번호 정규화, 문서 타입 분류 포함
...
1. AWS RDS 콘솔 로그인
2. 리전을 **ap-northeast-2 (서울)** 로 설정
3. 좌측 메뉴에서 **"스냅샷"** 클릭
4. **"수동 스냅샷"** 탭 선택
5. `ddoksori-final-2026-01-28-ver2` 스냅샷 선택 (최신 버전)
...
**DB 인스턴스 식별자**:
```
ddoksori-postgres-restored
```
- 또는 원래 이름 사용 (기존 RDS 삭제한 경우)

**DB 인스턴스 클래스** ⚠️ 중요!:
- ⭐ **db.r7g.xlarge** 선택 (4 vCPU, 32GB RAM)
- "메모리 최적화 클래스" 탭에서 찾기
- pgvector + 대량 데이터에 필수

**스토리지** ⚠️ 중요!:
- **스토리지 유형**: ⭐ **gp3 (범용 SSD)** 선택
- **할당된 스토리지**: ⭐ **100 GiB**
- 스냅샷이 gp2였어도 gp3로 변경 가능 (더 빠르고 저렴)
- 프로비저닝된 IOPS: 3000 (기본값)
- 스토리지 처리량: 125 MB/s (기본값)
...
**퍼블릭 액세스**:
- ⭐ **"예"로 설정 필수!**
- 팀원들이 외부에서 접속하려면 반드시 필요

**VPC 보안 그룹**:
- **기존 보안 그룹 선택** (5432 포트 열린 것)
- 또는 **새로 생성** (나중에 5432 포트 오픈 필요)
...
1. **"DB 인스턴스 복원"** 버튼 클릭 (주황색)
2. 복원 진행 대기 (5-10분)
3. 상태: "생성 중" → "백업 중" → **"사용 가능"**

---

#### 7단계: 엔드포인트 확인 및 연결 테스트

**엔드포인트 확인**:
1. RDS 콘솔에서 복원된 인스턴스 클릭
2. **"연결 & 보안"** 탭
3. **엔드포인트** 주소 복사
   - 예: `ddoksori-postgres-restored.xxxxx.ap-northeast-2.rds.amazonaws.com`

**보안 그룹 확인** (방법 A의 11단계 참조):
- 5432 포트가 0.0.0.0/0으로 열려있는지 확인
- 없다면 인바운드 규칙 추가 필요

**연결 테스트**:
```bash
psql -h [새-엔드포인트] -U postgres -d ddoksori -p 5432
# 원래 비밀번호 입력
```
...
# 2. 스냅샷에서 복원 (실제 스냅샷 예시)
aws rds restore-db-instance-from-db-snapshot \
  --db-instance-identifier ddoksori-postgres-restored \
  --db-snapshot-identifier ddoksori-final-2026-01-28-ver2 \
  --db-instance-class db.r7g.xlarge \
  --storage-type gp3 \
  --allocated-storage 100 \
  --publicly-accessible \
...
**db.t3.micro (1vCPU, 1GB RAM)** - 소규모 데이터만:
- 인스턴스: $15/월
- 스토리지 (gp3 20GB): $2.3/월
- 백업: 무료 (20GB 이내)
- **총 예상 비용**: **$17-20/월**

**db.r7g.xlarge (4vCPU, 32GB RAM)** ⭐ ddoksori 프로젝트용:
- 인스턴스: $280/월
- 스토리지 (gp3 100GB): $11.5/월
- 백업: 무료 (20GB 이내)
- **총 예상 비용**: **$291.5/월**
...
- **db.r6g.large** (2 vCPU, 16GB RAM): ~$140/월
- **db.t3.medium** (2 vCPU, 4GB RAM): ~$60/월
- **db.t3.small** (2 vCPU, 2GB RAM): ~$30/월

### 비용 절감 팁

1. **작업에 따라 인스턴스 크기 조절**:
   ```bash
   # 대량 데이터 삽입 시: r7g.xlarge
   aws rds modify-db-instance \
     --db-instance-identifier ddoksori-postgres \
     --db-instance-class db.r7g.xlarge \
     --apply-immediately

   # 일반 개발/조회만: t3.medium으로 다운그레이드
   aws rds modify-db-instance \
     --db-instance-identifier ddoksori-postgres \
     --db-instance-class db.t3.medium \
     --apply-immediately

   # 주의: 인스턴스 재시작 필요 (2-3분 다운타임)
   ```
